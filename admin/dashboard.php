...
if (isset($_POST['update_notice'])) {
    ...
    $chk_res = pg_query($dbconn, "SELECT id FROM notices WHERE id = 1");
    if ($chk_res && pg_num_rows($chk_res) > 0) {
        $res = pg_query_params($dbconn, "UPDATE notices SET content = $1 WHERE id = 1", array($content));
    } else {
        $res = pg_query_params($dbconn, "INSERT INTO notices (id, content) VALUES (1, $1)", array($content));
    }
...
// User Actions (Balance & Discount Update + Transaction Entry)
if (isset($_POST['update_user_details'])) {
    $u_id = (int)$_POST['user_id'];
    $new_bal = (float)$_POST['balance'];
    $disc = (int)$_POST['discount_percent'];
    
    // Fetch Old Balance to calculate difference
    $u_res = pg_query_params($dbconn, "SELECT balance FROM users WHERE id = $1", array($u_id));
    if ($u_res && pg_num_rows($u_res) > 0) {
        $u_data = pg_fetch_assoc($u_res);
        $old_bal = (float)$u_data['balance'];
        $diff = $new_bal - $old_bal;

        pg_query($dbconn, "BEGIN");
        try {
            $upd = pg_query_params($dbconn, "UPDATE users SET balance = $1, discount_percent = $2 WHERE id = $3", array($new_bal, $disc, $u_id));
            if (!$upd) { throw new Exception("Failed to update user."); }

            // Log Transaction if balance changed
            if (abs($diff) > 0.001) {
                $status_type = ($diff > 0) ? 'Approved' : 'Debited';
                $txn_id = 'ADM-' . strtoupper(substr(md5(uniqid(rand(), true)), 0, 8));
                $amount = abs($diff);

                pg_query_params(
                    $dbconn, 
                    "INSERT INTO payments (user_id, amount, txn_id, status, created_at) VALUES ($1, $2, $3, $4, NOW())", 
                    array((string)$u_id, $amount, $txn_id, $status_type)
                );
            }

            pg_query($dbconn, "COMMIT");
            $msg = "User balance & discount updated and logged to history successfully!";
        } catch (Exception $e) {
...
<head>
    ...
    /* কাস্টম ইনপুট এবং ব্যালেন্স প্রদর্শন সিএসএস */
    .balance-col { min-width: 320px; }
    .input-balance { min-width: 110px; font-weight: 600; }
    .input-discount { max-width: 75px; }
    </style>
</head>
<body>
                ...
                <table class="table table-bordered table-striped align-middle">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th>Username</th>
                            <th>Mobile</th>
                            <th class="balance-col">Balance & Special Discount (%)</th>
                            <th>Status</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                        ...
                        <?php if($users_res && pg_num_rows($users_res) > 0): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php if(($u['discount_percent'] ?? 0) > 0): ?>
                                        <br><span class="badge bg-success mt-1"><i class="fa-solid fa-tags me-1"></i><?php echo (int)$u['discount_percent']; ?>% Discount Active</span>
                                    <?php endif; ?>
                                </td>
                                <td><?php echo htmlspecialchars($u['mobile']); ?></td>
                                <td class="balance-col">
                                    <!-- বর্তমান ব্যালেন্স স্পষ্টভাবে দেখানোর জন্য টেক্সট -->
                                    <div class="mb-1 text-success fw-bold" style="font-size: 13px;">
                                        Current: ₹<?php echo number_format((float)$u['balance'], 2); ?>
                                    </div>
                                    <form method="POST" class="d-flex align-items-center">
                                        <input type="hidden" name="user_id" value="<?php echo $u['id']; ?>">
                                        <div class="input-group input-group-sm me-1" style="flex: 1;">
                                            <span class="input-group-text">₹</span>
                                            <input type="number" step="0.01" min="0" max="100000000" name="balance" value="<?php echo $u['balance']; ?>" class="form-control input-balance" title="Edit Balance">
                                        </div>
                                        <div class="input-group input-group-sm me-1 input-discount">
                                            <input type="number" name="discount_percent" value="<?php echo $u['discount_percent'] ?? 0; ?>" min="0" max="100" class="form-control" title="Discount Percentage (%)" placeholder="%">
                                            <span class="input-group-text">%</span>
                                        </div>
                                        <button type="submit" name="update_user_details" class="btn btn-sm btn-success">Save</button>
                                    </form>
                                </td>
