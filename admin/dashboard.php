...
// Balance Update with exact difference logic
    ...
    if ($u_res && pg_num_rows($u_res) > 0) {
        ...
        try {
            ...
            if (abs($diff) > 0.001) {
                ...
                $txn_prefix = ($diff > 0) ? 'ADM-' : 'ADM-DEBIT-';
                $txn_id = $txn_prefix . strtoupper(substr(md5(uniqid(rand(), true)), 0, 8));
                $amount = abs($diff);
            ...
            $msg = "User balance updated successfully! Logged " . ($diff >= 0 ? "+₹".number_format($diff, 2) : "-₹".number_format(abs($diff), 2)) . " to history.";
        } catch (Exception $e) {
...
<head>
    ...
    .input-balance { min-width: 120px; font-weight: 600; }
                            ...
                            <td class="balance-col">
                                ...
                                <form method="POST" class="d-flex align-items-center">
                                    ...
                                    <div class="input-group input-group-sm me-1" style="flex: 1;">
                                        ...
                                        <input type="number" step="0.01" min="0" max="100000000" name="balance" value="<?php echo $u['balance']; ?>" class="form-control input-balance" title="Set New Total Balance">
                                    </div>
