        ...
        if ($status === 'Canceled' || $status === 'Partial') {
            ...
            try {
                if ($refund_amount > 0) {
                    ...
                    // Log Refund to payments table
                    $txn_id = 'RFD-' . $order_id . '-' . rand(1000, 9999);
                    pg_query_params(
                        $dbconn,
                        "INSERT INTO payments (user_id, amount, txn_id, status, created_at) VALUES ($1, $2, $3, 'Refunded', NOW())",
                        array((string)$o['user_id'], $refund_amount, $txn_id)
                    );
                }
