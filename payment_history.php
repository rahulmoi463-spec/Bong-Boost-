...
function fetchTransactions($conn, $user_id) {
    // ডাটাবেস ফিল্টার: status = 'approved', column = created_at & charge/amount
    $sql = "
        (SELECT 'Deposit' AS type, amount, 'Payment Received' AS description, 'Credit' AS status, created_at 
         FROM payments 
         WHERE user_id = :uid1 AND status IN ('approved', 'completed', 'success'))
        UNION ALL
        ...
        (SELECT 'Refund' AS type, charge AS amount, CONCAT('Refund for Order #', id) AS description, 'Refund' AS status, created_at 
         FROM orders 
         WHERE user_id = :uid3 AND status IN ('cancelled', 'refunded'))
    ...
    if ($conn instanceof PDO) {
        ...
        $raw_sql = "
            (SELECT 'Deposit' AS type, amount, 'Payment Received' AS description, 'Credit' AS status, created_at 
             FROM payments 
             WHERE user_id = '$user_id' AND status IN ('approved', 'completed', 'success'))
            UNION ALL
            (SELECT 'Order' AS type, charge AS amount, CONCAT('Order #', id) AS description, 'Debit' AS status, created_at 
             FROM orders 
            ...
            (SELECT 'Refund' AS type, charge AS amount, CONCAT('Refund for Order #', id) AS description, 'Refund' AS status, created_at 
             FROM orders 
             WHERE user_id = '$user_id' AND status IN ('cancelled', 'refunded'))
            ORDER BY created_at DESC
        ";
