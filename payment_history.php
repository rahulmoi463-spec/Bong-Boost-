...
// PDO এবং MySQLi উভয় কানেকশনের জন্য ইউনিভার্সাল কুয়েরি প্রসেসর
function fetchTransactions($conn, $user_id) {
    $sql = "
        (SELECT 'Deposit' AS type, amount, 'Payment Received' AS description, 'Credit' AS status, created_at FROM payments WHERE user_id = :uid1 AND status = 'completed')
        UNION ALL
        (SELECT 'Order' AS type, charge AS amount, CONCAT('Order #', id, ' - ', service_name) AS description, 'Debit' AS status, created_at FROM orders WHERE user_id = :uid2)
        UNION ALL
        (SELECT 'Refund' AS type, charge AS amount, CONCAT('Refund for Cancelled Order #', id) AS description, 'Refund' AS status, updated_at AS created_at FROM orders WHERE user_id = :uid3 AND status IN ('cancelled', 'refunded'))
        ORDER BY created_at DESC
    ";

    // PDO ড্রাইভার সাপোর্ট (Supabase/Modern PHP)
    if ($conn instanceof PDO) {
        $stmt = $conn->prepare($sql);
        $stmt->execute([':uid1' => $user_id, ':uid2' => $user_id, ':uid3' => $user_id]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    } 
    // mysqli ড্রাইভার সাপোর্ট
    else if (function_exists('mysqli_query') && $conn) {
        $raw_sql = "
            (SELECT 'Deposit' AS type, amount, 'Payment Received' AS description, 'Credit' AS status, created_at FROM payments WHERE user_id = '$user_id' AND status = 'completed')
            UNION ALL
            (SELECT 'Order' AS type, charge AS amount, CONCAT('Order #', id, ' - ', service_name) AS description, 'Debit' AS status, created_at FROM orders WHERE user_id = '$user_id')
            UNION ALL
            (SELECT 'Refund' AS type, charge AS amount, CONCAT('Refund for Cancelled Order #', id) AS description, 'Refund' AS status, updated_at AS created_at FROM orders WHERE user_id = '$user_id' AND status IN ('cancelled', 'refunded'))
            ORDER BY created_at DESC
        ";
        $res = mysqli_query($conn, $raw_sql);
        $data = [];
        if ($res) {
...
$rows = fetchTransactions($conn, $user_id);
?>
