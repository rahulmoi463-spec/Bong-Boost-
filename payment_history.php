...
function fetchTransactions($db, $user_id) {
    ...
    $sql = "
        (SELECT 'Deposit' AS type, amount, CONCAT('Payment Received (Txn: ', txn_id, ')') AS description, 'Credit' AS status, created_at 
         FROM payments 
        ...
        (SELECT 'Refund' AS type, charge AS amount, CONCAT('Refund for Order #', id) AS description, 'Refund' AS status, created_at 
         FROM orders 
         WHERE user_id::text = $3 AND LOWER(status::text) IN ('cancelled', 'refunded'))
        ORDER BY created_at DESC
    ";

    // Supabase PostgreSQL (pg_query_params)
    if (function_exists('pg_query_params') && is_resource($db)) {
        $res = pg_query_params($db, $sql, array((string)$user_id, (string)$user_id, (string)$user_id));
        if ($res) {
