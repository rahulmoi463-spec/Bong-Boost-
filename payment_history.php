    ...
    if ($res1) {
        while ($r = pg_fetch_assoc($res1)) {
            $rows[] = [
                'type' => 'Deposit',
                'amount' => $r['amount'],
                'description' => 'Payment' . (!empty($r['txn_id']) ? ' (Txn: ' . $r['txn_id'] . ')' : ''),
                'status' => 'Credit',
...
<head>
    ...
    <style>
        ...
        th, td { padding: 6px 8px; border-bottom: 1px solid #2d3748; font-size: 11px; white-space: nowrap; }
        th { color: #8b949e; background: #0d1117; font-weight: 600; text-transform: uppercase; font-size: 10px; }
        
        .credit { color: #2ecc71; font-weight: bold; } 
        .debit { color: #e74c3c; font-weight: bold; }  
        
        .type-badge { padding: 2px 5px; border-radius: 3px; font-size: 10px; font-weight: 600; display: inline-block; }
        .bg-credit { background: rgba(46, 204, 113, 0.15); color: #2ecc71; }
        .bg-debit { background: rgba(231, 76, 60, 0.15); color: #e74c3c; }
        .bg-refund { background: rgba(52, 152, 219, 0.15); color: #3498db; }
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php elseif ($row['status'] == 'Debit'): ?>
                                        <span class="type-badge bg-debit">Order</span>
                                    <?php else: ?>
