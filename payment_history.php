...
// আপনার ফাইল স্ট্রাকচার অনুযায়ী config/db.php ইমপোর্ট করা হলো
...
// ১. Payments (Credit/Deposit) ডাটা সংগ্রহ
$payments_query = "SELECT 'Deposit' AS type, amount, 'Payment Received' AS description, 'Credit' AS status, created_at FROM payments WHERE user_id = '$user_id' AND status = 'completed'";

// ২. Orders (Debit/Spend) ডাটা সংগ্রহ
$orders_query = "SELECT 'Order' AS type, charge AS amount, CONCAT('Order #', id, ' - ', service_name) AS description, 'Debit' AS status, created_at FROM orders WHERE user_id = '$user_id'";

// ৩. Cancelled/Refunded Orders ডাটা সংগ্রহ
$refunds_query = "SELECT 'Refund' AS type, charge AS amount, CONCAT('Refund for Cancelled Order #', id) AS description, 'Refund' AS status, updated_at AS created_at FROM orders WHERE user_id = '$user_id' AND status IN ('cancelled', 'refunded')";

// ৪. সব ডাটা একসাথে করে তারিখ অনুযায়ী সাজানো (UNION ALL)
$final_query = "($payments_query) UNION ALL ($orders_query) UNION ALL ($refunds_query) ORDER BY created_at DESC";
