...
<head>
    ...
    <style>
        ...
        .glass-card { background: #ffffff; border: 1px solid #e2e8f0; border-radius: 20px; box-shadow: 0 4px 20px rgba(0,0,0,0.03); }
        .platform-card {
            background: #ffffff; border: 2px solid #e2e8f0; border-radius: 16px; padding: 14px 8px;
            text-align: center; cursor: pointer; transition: all 0.2s ease;
            display: flex; flex-direction: column; align-items: center; gap: 6px;
        }
        .platform-card:hover, .platform-card.active {
            border-color: #4f46e5 !important; background: #eef2ff !important; transform: translateY(-2px);
        }
        .platform-name { font-size: 13px; font-weight: 700; color: #1e293b; }
        ...
        .form-control, .form-select { 
            ...
            color: #0f172a !important; font-weight: 600 !important; border-radius: 12px !important; padding: 12px !important; 
        }
        .btn-3d-primary { 
            background: linear-gradient(135deg, #6366f1 0%, #4f46e5 100%); color: #fff; border: none; 
            border-radius: 12px; padding: 14px; font-weight: 700; box-shadow: 0 4px 15px rgba(99, 102, 241, 0.3);
        }
        .badge-balance { background: rgba(34, 197, 94, 0.1); color: #16a34a; border: 1px solid rgba(34, 197, 94, 0.3); font-size: 14px; padding: 8px 14px; border-radius: 10px; font-weight: 700; }
        .desc-box { background: #f1f5f9; border: 1px solid #cbd5e1; border-radius: 12px; padding: 14px; white-space: pre-line; font-size: 13px; color: #334155; }
    ...
    <div class="row justify-content-center">
        <div class="col-lg-7 col-md-9 col-sm-12">
            <div class="glass-card p-4">
                ...
                <!-- Direct Interactive Icon Selection -->
                <div class="mb-4">
                    <h5 class="fw-bold text-dark mb-3"><i class="fa-solid fa-shapes text-primary me-2"></i>Select Platform</h5>
                    <div class="row g-2" id="platformGrid">
                        <div class="col-4">
                            <div class="platform-card active" data-platform="instagram">
                        ...
                        <div class="col-4">
                            <div class="platform-card" data-platform="telegram">
                                <i class="fa-brands fa-telegram text-info fs-2"></i>
                        ...
                        <div class="col-4">
                            <div class="platform-card" data-platform="youtube">
                                <i class="fa-brands fa-youtube text-danger fs-2"></i>
                                <span class="platform-name">YouTube</span>
                            </div>
                        </div>
                        <div class="col-4">
                            <div class="platform-card" data-platform="tiktok">
                                <i class="fa-brands fa-tiktok text-dark fs-2"></i>
                ...
                <form method="POST">
                    ...
                    <!-- Smart Filtered Category Dropdown -->
                    <div class="mb-3">
                    ...
                    <div id="descContainer" class="mb-3 d-none">
                        <label class="form-label fw-bold text-primary"><i class="fa-solid fa-file-lines me-1"></i> Details</label>
...
<script>
    ...
    // Filter Categories based on selected Platform Icon
    function filterCategoriesByPlatform(platform) {
        ...
        Object.keys(servicesData).forEach(cat => {
            if (cat.toLowerCase().includes(platform.toLowerCase())) {
        ...
        // Auto select the first filtered category if available
        if (matched && catSelect.options.length > 1) {
            catSelect.selectedIndex = 1;
            populateServices(catSelect.value);
        }
    }

    // Populate services under selected category
    ...
    // Icon Grid Click Event
    document.querySelectorAll('.platform-card').forEach(card => {
        card.addEventListener('click', function() {
            document.querySelectorAll('.platform-card').forEach(c => c.classList.remove('active'));
            this.classList.add('active');
            filterCategoriesByPlatform(this.dataset.platform);
        });
    });
    ...
    // Default Initialization on Load (Instagram Selected)
    filterCategoriesByPlatform('instagram');
</script>
