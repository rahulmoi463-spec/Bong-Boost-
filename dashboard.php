...
// Try Fetching Live Services via API
$ch = curl_init();
...
$cache_v = time(); // Dynamic Cache Busting
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<!-- Side Drawer -->
<div class="offcanvas offcanvas-end offcanvas-custom" tabindex="-1" id="sidebarMenu">
  <div class="offcanvas-header border-bottom border-secondary">
    <div class="d-flex align-items-center gap-2">
      ...
      <div>
        <h6 class="offcanvas-title fw-bold text-white mb-0"><?php echo htmlspecialchars($user['username']); ?></h6>
  ...
  <div class="offcanvas-body d-flex flex-column justify-content-between p-3">
    <div>
        <a href="dashboard.php" class="drawer-link active"><i class="fa-solid fa-cart-plus"></i> New Order</a>
        <a href="add_fund.php" class="drawer-link"><i class="fa-solid fa-wallet"></i> Add Fund</a>
        <a href="free_views.php" class="drawer-link text-warning"><i class="fa-solid fa-gift"></i> Free Views / Gifts</a>
        <a href="orders_history.php" class="drawer-link"><i class="fa-solid fa-clock-rotate-left"></i> Order History</a>
        <a href="https://wa.me/917718231993?text=Hello%20Bong%20Boost%20Support" target="_blank" class="drawer-link text-success"><i class="fa-brands fa-whatsapp"></i> Customer Support</a>
    </div>

    <div>
        <hr class="border-secondary mb-3">
        <a href="logout.php" class="drawer-link text-danger"><i class="fa-solid fa-right-from-bracket"></i> Logout</a>
    </div>
  </div>
</div>
...
<script>
    ...
    document.querySelectorAll('.platform-card').forEach(card => {
        card.addEventListener('click', function() {
            ...
            searchInput.value = this.dataset.platform;
    ...
    function populateServices(selectedCat, keyword = '') {
        ...
        if (selectedCat && servicesData[selectedCat]) {
            servicesData[selectedCat].forEach(srv => {
                let match = filterKw === '' || (srv.id + " " + srv.name + " " + (srv.desc || '')).toLowerCase().includes(filterKw);
                if (match) {
            ...
            Object.keys(servicesData).forEach(cat => {
                servicesData[cat].forEach(srv => {
                    if ((cat + " " + srv.id + " " + srv.name + " " + (srv.desc || '')).toLowerCase().includes(filterKw)) {
                        const opt = document.createElement('option');
                        opt.value = srv.id;
                        opt.textContent = "[" + srv.id + "] " + srv.name + " - ₹" + parseFloat(srv.rate).toFixed(2);
                        opt.dataset.name = srv.name;
    ...
    catSelect.addEventListener('change', function() { populateServices(this.value, searchInput.value); });
    ...
    srvSelect.addEventListener('change', function() {
        ...
        if(opt && opt.value) {
            ...
            if (opt.dataset.cat) { catSelect.value = opt.dataset.cat; }
