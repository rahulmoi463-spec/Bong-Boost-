...
// Handle Refill Request Safely with PRG Pattern
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['request_refill'])) {
    ...
    if ($ord_check && pg_num_rows($ord_check) > 0) {
        ...
        if ($ord['status'] == 'Refill Requested') {
            $_SESSION['flash_error'] = "Refill request is already pending!";
        } else {
            pg_query_params($dbconn, "UPDATE orders SET status = 'Refill Requested' WHERE id = $1", array($order_id));
            $_SESSION['flash_success'] = "Refill request submitted successfully!";
        }
    } else {
        $_SESSION['flash_error'] = "Invalid order details.";
    }
...
<head>
    ...
    <style>
        ...
        /* মোবাইল ও স্ক্রিনে সার্ভিস নেম ভেঙে যাওয়া রোধের সিএসএস */
        .custom-orders-table {
            min-width: 900px; /* টেবিলের ন্যূনতম উইডথ নির্দিষ্ট করা হলো */
        }

        .service-name-col {
            min-width: 250px; /* সার্ভিস নেমের জন্য পর্যাপ্ত জায়গা বরাদ্দ */
            white-space: normal;
            word-wrap: break-word;
        }

        .nowrap-col {
            white-space: nowrap; /* লেখাগুলোকে এক লাইনে রাখা */
        }
...
<div class="container pb-5">
    <div class="glass-card p-4 p-md-5">
        ...
        <div class="table-responsive">
            <table class="table table-hover table-striped align-middle custom-orders-table" id="ordersTable">
                <thead class="table-dark">
                    <tr>
                        <th class="nowrap-col">ORDER ID</th>
                        ...
                        <th style="min-width: 170px;">STATUS & PROGRESS</th>
                        <th class="nowrap-col">ACTION</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if ($orders_query && pg_num_rows($orders_query) > 0): ?>
                        <?php while ($row = pg_fetch_assoc($orders_query)): 
                            $display_id = !empty($row['custom_order_id']) ? $row['custom_order_id'] : $row['id'];
                        ?>
                            <tr>
                                <td class="nowrap-col">
                                    <span class="fw-bold"><?php echo htmlspecialchars($display_id); ?></span>
                                    <button type="button" class="copy-btn" onclick="copyOrderId('<?php echo htmlspecialchars($display_id); ?>')" title="Copy ID">
                                ...
                                <td class="service-name-col fw-semibold">
                                    <?php echo htmlspecialchars(!empty($row['service_name']) ? $row['service_name'] : ('Service #' . $row['service_id'])); ?>
                                </td>
                                ...
                                <td class="nowrap-col"><?php echo number_format($row['quantity']); ?></td>
                                <td class="text-success fw-bold nowrap-col">₹<?php echo number_format($row['charge'], 2); ?></td>
                                <td>
                                    <?php 
                                    ...
                                    ?>
                                    <div class="py-1">
                                        <span class="badge <?php echo $badge_class; ?> mb-2 px-2 py-1 shadow-sm"><?php echo htmlspecialchars($row['status']); ?></span>
                                        <div class="progress" style="height: 8px; background-color: #cbd5e1; border-radius: 10px;">
...
<script>
    ...
    <?php if(isset($_SESSION['flash_success'])): ?>
        Swal.fire({
            icon: 'success',
            title: 'Submitted!',
            text: '<?php echo $_SESSION['flash_success']; unset($_SESSION['flash_success']); ?>',
    ...
    <?php if(isset($_SESSION['flash_error'])): ?>
        Swal.fire({
            icon: 'error',
            title: 'Notice',
            text: '<?php echo $_SESSION['flash_error']; unset($_SESSION['flash_error']); ?>',
